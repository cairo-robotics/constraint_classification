...
bool upright(geometry_msgs::Pose upright_pose, geometry_msgs::Pose current_pose, float threshold_angle)
{
  ...
  // We must transform a reference vector by the upright quaternion and the current quaternion and get the angle between the resulting vecotrs.
  // The angle between quaternions can be affected by yaw.
